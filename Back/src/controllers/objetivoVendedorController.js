...
export async function postObjetivoVendedor(req, res) {
  try {
    ...
    if (!scope) return
    const data = await createSellerLifeGoal(applyScopeToPayload(req.body, scope))
...
export async function putObjetivoVendedor(req, res) {
  try {
    ...
    if (!scope) return
    const data = await updateSellerLifeGoal(req.params.id, applyScopeToPayload(req.body, scope))
    return res.json(data)
  } catch (error) {
